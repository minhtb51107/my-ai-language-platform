...
import { Repository, DataSource } from 'typeorm'; // Thêm DataSource
...
export class MemoryService {
  ...
  constructor(
    ...
    private readonly dataSource: DataSource, // Inject DataSource để chạy Raw SQL siêu tốc
  ) {}
  ...
  async searchRelevantKnowledge(userId: string, query: string, topK: number = 3): Promise<string> {
    try {
      ...
      // Kết hợp 70% điểm Vector và 30% điểm Keyword
      const querySql = `
      ...
      // 2. STAGE 2: Cohere Reranking giữ nguyên vì nó vẫn là "vị vua" chấm điểm ngữ cảnh
  ...
  // Lấy các trí nhớ do người dùng chủ động lưu
